...
export default function DashboardPage({ params }: { params: { userId: string } }) {
  ...
  useEffect(() => {
    ...
    if (storedUserId !== params.userId) return router.push('/login')

    setToken(storedToken)
    fetch(`/api/user?userId=${params.userId}`)
      .then(r => r.json())
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b px-6 py-4 flex items-center justify-between">
        ...
        <span className="text-sm text-muted-foreground">{params.userId.slice(0, 8)}...</span>
      </header>
      ...
      <main className="max-w-3xl mx-auto p-6 space-y-8">
        ...
        <section>
          ...
          <div className="bg-muted rounded-lg p-4 space-y-2 text-sm">
            ...
            <p><span className="text-muted-foreground">User ID:</span> <code className="bg-background px-1.5 py-0.5 rounded text-xs">{params.userId}</code></p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="bg-muted rounded-lg p-4">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Token is shown only once after registration. Save it securely.
            </p>
          </div>
        </section>
          ...
          {canvases.length === 0 ? (
            ...
            <div className="space-y-2">
              {canvases.map((c: any, i: number) => (
                <a key={i} href={`/${params.userId}/${c.name}`}
